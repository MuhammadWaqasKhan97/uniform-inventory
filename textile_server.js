...
app.post("/job_slip/cutting",async (req,res)=>{
    ...
    let duplicate = false;
    let js = null;
     let js_array = await Job_Slip.find({ref:lpo_ref,starting_number:{$lt:barcode_array[0]}});
     js_array.forEach((j)=>{
...
app.post('/barcode_info', async (req,res)=>{
    ...
    let cutting = false,
        ...
        qc = false,
        packing = false;
   let js_array = await Job_Slip.find({starting_number:{$lt:bc_num}});
   js_array.forEach((j)=>{
...
app.post('/generate_sales_invoice',(req,res)=>{
    let ref = req.body.ref,
        ...
        Invoices.findOne({ref:ref},async (err,invoice)=>{
            ...
            tax = lpo.tax,
            items_array = JSON.parse(lpo.items_array);
